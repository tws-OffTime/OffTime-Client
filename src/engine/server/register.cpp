...
class CRegister : public IRegister
{
	...
	class CProtocol
	{
		...
		class CJob : public IJob
		{
			...
			std::unique_ptr<CPostJson> m_pRegister;
			virtual void Run();

		public:
			CJob(int Protocol, int Index, int InfoSerial, std::shared_ptr<CShared> pShared, std::unique_ptr<CPostJson> &&pRegister) :
				m_Protocol(Protocol),
				m_Index(Index),
...
void CRegister::CProtocol::SendRegister()
{
	...
	std::unique_ptr<CPostJson> pRegister = std::make_unique<CPostJson>(m_pParent->m_pConfig->m_SvRegisterUrl, aJson, CTimeout{0, 0, 0}, HTTPLOG::FAILURE, ProtocolToIpresolve(m_Protocol));
